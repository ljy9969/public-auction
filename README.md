...
3. **인근 지번** (`jibun`) — **같은 단지 거래가 12개월 내 0건일 때** 쓰는 '주변' 폴백. 조건: ① 같은 법정동(umdNm) ② **지번 본번 ±10** (`JIBUN_NEAR_BONBUN`, 좌표 대용 근접도 — 본번은 대체로 순차 배정) ③ **면적 ±10%** (`JIBUN_AREA_TOL`, 다른 건물이라 보수적). 단지명 매칭과 달리 **primary endpoint(주용도) 거래로만 제한**해 타용도 혼입 방지
